 ...
 */
 ...
 #include "images.h"  // Include custom images
  ...
  const char* ssid = "Hacked";              //WIFI SSID Name                             
  const char* password =  "0123456789";        //WIFI Password
  const char* host = "api.thingspeak.com";  //We read the data from this host                                   
  const int httpPortRead = 80;                                           
                                                  
  const char* url1 = "/apps/thinghttp/send_request?api_key=2QBXYS6ABF9RGJUW";     //Change this URL Cases
  const char* url2 = "/apps/thinghttp/send_request?api_key=EH6HD2BUFHZGJ0EH";   //Deaths
  const char* url3 = "/apps/thinghttp/send_request?api_key=HY1XWXRQQPAC5HQ0";   //Recovered
